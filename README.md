...
However, be careful, connecting a child process to the controlling terminal triggers both the signals `SIGTTIN` and `SIGTTOU` (one for the standard input, one for the standard output). Their immediate effect to prematurely stop it, before it even had the chance to execute your command. As solution is to deactivate these signals just before forking. Everything is now setup, the shell can execute the C program with the API call `os.execvp`. 
